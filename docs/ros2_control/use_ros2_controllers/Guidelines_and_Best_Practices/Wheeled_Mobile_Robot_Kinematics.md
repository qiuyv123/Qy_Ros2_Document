...
$$
\dot{x} = v_{\text{rear}} \cos(\theta),\quad
\dot{y} = v_{\text{rear}} \sin(\theta),\quad
...
$$
\dot{x} = v_{\text{front}} \cos(\theta) \cos(\phi),\quad
\dot{y} = v_{\text{front}} \sin(\theta) \cos(\phi),\quad
...
!!! note "注意"
    阿克曼转向也可通过**机械连杆**实现（仅一个转向输入），此时运动学与类车模型相同。
